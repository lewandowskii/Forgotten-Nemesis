...
Component({
  ...
  attached: async function () {
    ...
    console.log('attached: chatMode', chatMode);
    // 检查配置
    ...
    if (chatMode === "bot") {
      ...
      this.setData({
        bot,
        questions,
        chatRecords: chatRecords.length > 0 ? chatRecords : [],
  ...
  methods: {
    ...
    getUserContext: function () {
      ...
      const anniversaries = memories.map(memory => {
        ...
        const daysSince = Math.floor((today - targetDate) / (1000 * 60 * 60 * 24));
        
        let ordinal = today.getFullYear() - targetDate.getFullYear();
        if (memory.repeatYearly) {
            ordinal = nextDate.getFullYear() - targetDate.getFullYear();
        }

        return {
          name: memory.title,
          date: memory.targetDate,
          type: memory.type,
          daysUntil: daysUntil,
          daysSince: daysSince,
          ordinal: ordinal
    ...
    sendImplicitContext: async function () {
      console.log('默认消息发送')
      const context = this.getUserContext();
      // 这里必须使用 JSON.stringify 将对象转为字符串发送
      const message = JSON.stringify(context);
      if (this.data.isAgent) {
        await this.handleSendMessageV2(message, { isHidden: true });
      } else {
        await this.sendMessage(message, { isHidden: true });
      }
    },
    ...
    handleUserMessage: function (message, options = {}) {
      const { inputValue } = this.data;
      const content = message || inputValue;
      const userMsg = { id: "user_message_" + Date.now(), role: "user", content, isHidden: options.isHidden || false };
      // 为了保证格式统一，返回数组
      return [userMsg];
    },
    ...
    handleSendMessageV2: async function (message, options = {}) {
      const userMessages = this.handleUserMessage(message, options);
      this.setData({
        chatStatus: 1, // 状态：生成中
        inputValue: "",
      });
